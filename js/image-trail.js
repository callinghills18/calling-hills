...
// Including Variant 1 (can include others if needed, but 1 is usually a good default)
// For brevity and since the user provided 8 variants, I will include Variant 1 and maybe 5 (which is very cool).
...
function initImageTrail(containerSelector, items = [], variant = 1) {
  const container = document.querySelector(containerSelector);
  if (!container) return;

  // Generate HTML for items
  container.innerHTML = items.map((url, i) => `
    <div class="content__img" data-index="${i}">
      <div class="content__img-inner" style="background-image: url('${url}')"></div>
    </div>
  `).join('');
